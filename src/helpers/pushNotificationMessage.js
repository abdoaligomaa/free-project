...
export async function pushNotificationMessage(actionType, notifyContent) {
  ...
  if (actionType === 'newEnquiry') {
    title = `New Enquiry`;
    message = `${formatText(notifyContent['userName'])}: ${
  ...
  if (actionType === 'listApproved') {
    ...
    message = `${formatText(
      notifyContent['userName'],
    )} : The Admin has verified your listing ${
      notifyContent['listTitle']
    } . Please publish your listing to get reservations.`;
  }
  ...
  if (actionType === 'listDeclined') {
    ...
    message = `${formatText(
      notifyContent['userName'],
    )} : Admin has declined your listing request for the ${
      notifyContent['listTitle']
    } due to the reason of ${
      notifyContent['reason']
    }. Kindly update the listing information and submit for the list approval.`;
  }
