...
import { Repost, Reservation, TransactionHistory } from '../../models';
...
const getTransactionHistory = {
  ...
  async resolve(source, { mode, payoutId, listId, currentPage }, { request } , cccc) {
    const limit = 5;
    let offset = 0;
    ...
    if (request.user && !request.user.admin) {
      ...
      let repostsId = sequelize.literal(`SELECT repost_id from Reposts_Share where host_id=${hostId}`);
      ...
      const repostData = await Repost.findAll({
        where: {
          id: {$in: [repostsId]}
        },
      });
