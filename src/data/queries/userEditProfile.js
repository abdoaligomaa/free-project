...
// Sequelize models
import { User, UserProfile, AdminUser, EmailToken } from '../../data/models';
  ...
  async resolve(
    ...
    if (request.user && request.user.admin != true) {
      ...
      let isPhoneNumber = phoneNumber
        ? phoneNumber
        : isUserPhoneNumber.phoneNumber;
      let isCountryCode = countryCode
        ? countryCode
        : isUserPhoneNumber.countryCode;
      let isCountryName = countryName
        ? countryName
        : isUserPhoneNumber.countryName;
      ...
      if (loggedinEmail != email) {
        ...
        if (getUserId) {
          ...
          const token = jwt.sign(
            { id: request.user.id, email: email },
            auth.jwt.secret,
            { expiresIn },
          );
