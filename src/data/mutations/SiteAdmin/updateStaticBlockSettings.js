...
const updateStaticBlockSettings = {
  ...
  async resolve(source,{
    ...
    if (request.user && request.user.admin == true) {
      ...
      // Header Info
      const updateHeader = await StaticInfoBlock.update(
        { content: headerContent, title: headerTitle, isEnable: isEnable },
        { where: { name: 'header' } },
      ...
      const updateBlock1 = await StaticInfoBlock.update(
        { title: blockTitle1, content: blockContent1, image: blockImage1 },
        { where: { name: 'block1' } },
      ).then(function (instance) {
        // Check if any rows are affected
        if (instance > 0) {
      ...
      const updateBlock2 = await StaticInfoBlock.update(
        { title: blockTitle2, content: blockContent2, image: blockImage2 },
        { where: { name: 'block2' } },
      ).then(function (instance) {
        // Check if any rows are affected
        if (instance > 0) {
