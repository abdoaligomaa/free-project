...
import { payment as config } from '../../../config-sample';
            ...
            async function (error, payout) {
              if (error) {
                res.send({
                  ...
                  errorMessage: error.response && error.response.message,
                });
                throw error;
              } else {
                ...
                var fees =
                  payout.batch_header.fees && payout.batch_header.fees.value;
                if (batchStatus && batchStatus === 'SUCCESS') {
                  ...
                  if (batchStatus === 'PENDING') {
                    let getPayoutInfo = paypal.payout.get(
                      batchId,
                      async function (getError, getResponse) {
                        if (getError) {
                          res.send({
                            status: getError.response,
                            errorMessage:
                              getError.response && getError.response.message,
                          });
                          //throw getError;
                        } else {
                          ...
                          if (
                            getResponse &&
                            getResponse.batch_header &&
                            (batchStatus === 'PENDING' ||
                              batchStatus === 'SUCCESS')
                          ) {
                            fees =
                              getResponse.batch_header.fees &&
                              getResponse.batch_header.fees.value;
