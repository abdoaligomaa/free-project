export function getSubject(type, data = null) {
  let subject, previewText;

  if (type === 'subGuestShareReservation') {
    const { guest } = data 
    subject = 'Trip share invitation';
    previewText = 'You have been invited to share ' + guest + '\'s trip';
  }
  if (type === 'subGuestPayAmount') {
    const { trip } = data
    subject = 'Trip amount payment';
    previewText = 'You have received amount payment for ' + trip;
  }
  ...
  if (type === 'reportUser') {
    ...
    previewText =
      'You got a notification for user violation. Reporting by someone.';
  }
