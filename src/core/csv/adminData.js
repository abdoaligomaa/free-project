import { User, UserProfile, Listing, Reservation } from '../../data/models';
import sequelize from '../../data/sequelize';
import moment from 'moment';

function getQuery(column, separator) {
  return sequelize.literal(
    `IF(${column} IS NULL OR ${column}="", "", CONCAT(${column},${separator}))`,
  );
}
...
export async function users(keyword, userType) {
        ...
        $or: [
          {
            $in: [
              sequelize.literal(
                `SELECT userId FROM UserProfile WHERE firstName like '%${keyword}%'`,
              ),
            ],
          },
          {
            $in: [
              sequelize.literal(
                `SELECT userId FROM UserProfile WHERE lastName like '%${keyword}%'`,
              ),
            ],
          },
          {
            $in: [
              sequelize.literal(
                `SELECT userId FROM UserProfile WHERE phoneNumber like '%${keyword}%'`,
              ),
            ],
          },
          {
            $in: [
              sequelize.literal(
                `SELECT userId FROM UserProfile WHERE createdAt like '%${getDate}%'`,
              ),
            ],
          },
          {
            $in: [
              sequelize.literal(
                `SELECT id FROM User WHERE email like '%${keyword}%'`,
              ),
            ],
          },
        ],
  ...
  const dataItems = await User.findAll({
    ...
    where: {
      $and: [{ userDeletedAt: null }, userTypeFilter, keywordFilter],
    },
    include: [
      {
        model: UserProfile,
...
export async function reservations(search) {
  try {
    ...
    let paymentStateFilter = { paymentState: 'completed' },
      searchFilter = {};
        ...
        $or: [
          ...
          { id: { $like: '%' + search + '%' } },
          {
            listId: {
              $in: [
                sequelize.literal(
                  `SELECT id FROM Listing WHERE title like '%${search}%'`,
                ),
              ],
            },
          },
    ...
    const result = await Reservation.findAll({
      ...
      where: {
        $and: [paymentStateFilter, searchFilter],
      },
  ...
  try {
        ...
        $or: [
          ...
          { buildingName: { $like: '%' + search + '%' } },
          {
            createdAt: {
              $in: [
                sequelize.literal(
                  `SELECT createdAt FROM Listing WHERE createdAt like '%${getDate}%'`,
                ),
              ],
            },
          },
    ...
    const result = await Listing.findAll({
      attributes: [
        ...
        [
          sequelize.literal(
            `(SELECT firstName FROM UserProfile WHERE userId=Listing.userId)`,
          ),
          'Host Name',
        ],
        [
