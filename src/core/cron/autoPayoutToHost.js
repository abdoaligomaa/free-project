    ...
    async function () {
      ...
      async function autoPayout(offset) {
        try {
          ...
          const completedReposts = await Repost.findAll({
            attributes: 'id',
            where:{
          ...
          const completedRepostsAndNotPaid = await Repost_Share.findAll({
            where: {
              repost_id: {
                $in: completedReposts
              },
          ...
          if (completedRepostsAndNotPaid && completedRepostsAndNotPaid.length > 0)
          {
            await Promise.all(
              completedRepostsAndNotPaid.map(async (item, index) => {
                await AllowedLimit();
                ...
                const newPrice = await Repost.findOne({
                  attributes: 'new_night_price',
                  where:{
                    id: item.repost_id
                  }
                })
                let amount = newPrice.toFixed(2);
                if (checkUserStatus != null)
                {
                  try {
                    let payout = await stripe.transfers.create({
                      amount: Math.round(amount),
                      destination: checkUserStatus.email,
                    ...
                    if (payout && payout.id) {
                      const createTransaction = await TransactionHistory.create({
                        reservationId: item.reservation_id,
                        userId: item.host_id,
                        payoutEmail: checkUserStatus.email,
                        amount: Math.round(amount),
                      });
              ...
              getReservation.map(async (item, index) => {
                ...
                if (
                  ...
                  if (status == 400) {
                    let updateFailed;
                    checkFailedTransaction =
                      await FailedTransactionHistory.findOne({
                        where: {
                          reservationId: item.id,
                        },
                        raw: true,
