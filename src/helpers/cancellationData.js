...
export function cancellationGuestData(
  ...
  let updatedHostFee = 0,
    ...
    hostRefund = 0;
  let checkInNights =
    remainingNights == 0 || remainingNights > 0 ? remainingNights : nights;
  ...
  refundableNightPrice = getFixedValue(
    totalNights * basePrice * (accomodation / 100) + isCleaingPrice,
  );
  ...
  if (hostRefund > 0) {
    updatedHostFee = getFixedValue(
      hostServiceFeeType === 'percentage'
        ? hostRefund * (Number(hostServiceFeeValue) / 100)
        : hostServiceFeeValue,
    );
