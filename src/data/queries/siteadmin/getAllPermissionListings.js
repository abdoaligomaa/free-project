              ...
              $or: [
                {
                  $in: [
                    sequelize.literal(
                      `SELECT id FROM Listing WHERE id LIKE '%${searchList}%'`,
                    ),
                  ],
                },
                {
                  $in: [
                    sequelize.literal(
                      `SELECT id FROM Listing WHERE userId IN (SELECT id FROM User WHERE email LIKE '%${searchList}%')`,
                    ),
                  ],
                },
                {
                  $in: [
                    sequelize.literal(
                      `SELECT id FROM Listing WHERE title LIKE '%${searchList}%'`,
                    ),
                  ],
                },
                {
                  $in: [
                    sequelize.literal(
                      `SELECT id FROM Listing WHERE userId IN (SELECT userId FROM UserProfile WHERE firstName LIKE '%${searchList}%')`,
                    ),
                  ],
                },
                {
                  $in: [
                    sequelize.literal(
                      `SELECT id FROM Listing WHERE createdAt LIKE '%${searchList}%'`,
                    ),
                  ],
                },
              ],
