...
const updateSideMenu = {
  ...
  async resolve(source,{
    ...
    if (request.user && request.user.admin == true) {
      ...
      const updateBlock1 = await SideMenu.update(
        { title: blockTitle1, description: blockContent1 },
        { where: { name: 'block1' } },
      ...
      const updateBlock2 = await SideMenu.update(
        { title: blockTitle2, description: blockContent2 },
        { where: { name: 'block2' } },
      ).then(function (instance) {
        // Check if any rows are affected
        if (instance > 0) {
      ...
      // Block 2 Info
      // if (blockTitle2 && blockContent2 && blockImage2) {
      const updateBlock3 = await SideMenu.update(
        { title: blockTitle3, description: blockContent3 },
        { where: { name: 'block3' } },
      ).then(function (instance) {
        // Check if any rows are affected
        if (instance > 0) {
