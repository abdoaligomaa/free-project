...
import { payment, url } from '../../../config-sample';
...
const stripeAddPayout = app => {
  app.post('/stripe-add-payout', async function (req, res) {
    ...
    if (status === 200) {
      try {
        business_type =
          bankDetails && bankDetails.businessType
        ...
        if (
          !isEuropeCountry(bankDetails.country) &&
          bankDetails.routingNumber
        ) {
          // Non Europe countries - Routing Number param
          requestData['external_account']['routing_number'] =
            bankDetails.routingNumber;
        }
  ...
  app.post('/stripe-verify-payout', async function (req, res) {
    ...
    try {
      ...
      let accountUserId =
        getAccountDetails &&
        getAccountDetails.metadata &&
        getAccountDetails.metadata.userId;

      if (userId === accountUserId) {
        const accountVerifyPendingVerifications =
        ...
        redirect =
          accountVerifyPendingVerifications &&
          accountVerifyPendingVerifications.url;
      } else {
        status = 400;
