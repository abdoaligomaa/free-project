...
const content = {
  ...
  async resolve(source, { path, locale }, { request } , cccc) {
    const { success, fileName, extension } = await resolveFileName(
      path,
      locale,
    );
    if (!success) {
    ...
    const source2 = await readFile(fileName, { encoding: 'utf8' });
    return parseContent(path, source2, extension);
  },
};
