    ...
    async (req, res, next) => {
      ...
      const xSmallImage = await new Promise((resolve, reject) => {
        sharp(files[0].path)
          ...
          //.crop(sharp.strategy.entropy)
          .toFile(
            fileuploadDir + 'x_small_' + files[0].filename,
            function (err) {
              if (files) {
                resolve(files);
              } else {
      ...
      if (files[0].mimetype === 'image/jpeg') {
        ...
        const mediumImage = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            //.crop(sharp.strategy.entropy)
            .toFile(
              fileuploadDir + 'x_medium_' + files[0].filename,
              function (err) {
                if (files) {
                  resolve(files);
                } else {
        ...
        const largeImage = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            //.crop(sharp.strategy.entropy)
            .toFile(
              fileuploadDir + 'x_large_' + files[0].filename,
              function (err) {
                if (files) {
                  resolve(files);
                } else {
        ...
        const largeImageSize = await new Promise((resolve, reject) => {
          sharp(files[0].path)
            ...
            //.crop(sharp.strategy.entropy)
            .toFile(
              fileuploadDir + 'xx_large_' + files[0].filename,
              function (err) {
                if (files) {
                  resolve(files);
                } else {
