...
import { url as siteURL } from '../../config-sample';
...
const sitemapRoutes = (app, routes) => {
  app.get('/sitemap.xml', async function (req, res) {
    ...
    let staticData = await Promise.all(
      routes.children.map(async (value, key) => {
        if (
          !value.path.includes('/siteadmin') &&
          unwantedURLs.indexOf(value.path) < 0
        ) {
          staticURLs.push({
            url: value.path,
            changefreq: 'daily',
    ...
    let listingDataURLs = await Promise.all(
      listingData.map(async value => {
        staticURLs.push({
          url: value,
          changefreq: 'daily',
          priority: 0.7,
