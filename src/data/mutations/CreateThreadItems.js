...
const CreateThreadItems = {
  ...
  async resolve(source,{ listId, host, content, type, startDate, endDate, personCapacity }, { request, response }, ccc) {
      ...
      if (thread) {
        ...
        if (threadItems) {
          ...
          if (getThread && getThread.host && getThread.guest) {
            notifyUserId =
              getThread.host === userId ? getThread.guest : getThread.host;
            notifyUserType = getThread.host === userId ? 'guest' : 'host';
            notifyGuestId =
              getThread.host === userId ? getThread.guest : getThread.host;
            notifyHostId =
              getThread.host === userId ? getThread.host : getThread.guest;
          }
          ...
          if (guestProfile && getThread) {
            userName =
              guestProfile && guestProfile.firstName
          ...
          let notifyContent = {
            ...
            guestPicture:
              guestProfile && guestProfile.picture
                ? guestProfile.picture.toString()
                : '',
            hostId: notifyHostId.toString(),
            hostName: hostProfile && hostProfile.firstName.toString(),
            hostPicture:
              hostProfile && hostProfile.picture
                ? hostProfile.picture.toString()
                : '',
