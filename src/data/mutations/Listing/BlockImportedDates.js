...
const blockImportedDates = {
  ...
  async resolve(source, { calendarId, listId, blockedDates }, { request } , cccc) {
        ...
        if (blockedDates && blockedDates.length > 0) {
          ...
          if (reservationDates && reservationDates.length > 0) {
            reservationDates.map(item => {
              reservationDatesCollection.push({
          ...
          blockedDates.map(async item => {
            let oldRecords = reservationDatesCollection.find(
              v =>
                moment(item).format('YYYY-MM-DD') ===
                moment(v.blockedDates).format('YYYY-MM-DD'),
            );
            ...
            if (checkDate === -1 && !oldRecords) {
              ...
              blockedDatesInstance = {
                ...
                listId,
                calendarId,
                blockedDates: moment(oldRecords.blockedDates).format(
                  'YYYY-MM-DD',
                ),
                calendarStatus: 'blocked',
              };
          ...
          const bulkCreate = await ListBlockedDates.bulkCreate(
